...
Item {
    ...
    /*收到Music Service 媒體資訊*/
    Connections {
        target: qtAndroidService
        function onMediaInfo(trackName, artistName, albumName, duration) {
    ...
    Connections {
        target: qtAndroidService
        function onMediaThumbPath(thumbPath) {
    ...
    Connections {
        target: qtAndroidService
        function onRadioInfo(currIndex,currTitle,currFreq,isUpdateList,radio_list){
